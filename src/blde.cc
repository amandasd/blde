...
void blde_init( int argc, char** argv ) 
{
   ...
   sdata.num_generation_leader = Opts.Int.Get("-gl");
...
#ifdef PROFILING
   sdata.time_init = t_init.elapsed();
#endif
}
...
int best_individual(int idx, real_t* fit_popL, real_t* fit_popLValoresF)
{
   ...
   for( int i = 1; i < sdata.population_leader_size; i++ )
   {
...
{
#ifdef PROFILING
   sdata.time_follower = 0.0;
   sdata.time_leader   = 0.0;
#endif
...
//#endif
   ...
   acc_seed();
#ifdef PROFILING
   sdata.time_seed = t_seed.elapsed();
#endif
...
#ifdef PROFILING
      sdata.time_leader += t_leader.elapsed();
#endif
...
//#if ! defined( PROFILING )
      ...
      double eps = 0.;
      for( int j = 0; j < sdata.leader_dimension; j++ ){
         eps += popL[idx + j * sdata.population_leader_size] * popL[idx + j * sdata.population_leader_size];
      }
...
#ifdef PROFILING
   printf("time_init: %lf, time_seed: %lf, time_initialization: %lf, time_follower: %lf, time_leader: %lf, gen: %d, stop_eps: %d\n", sdata.time_init, sdata.time_seed, sdata.time_initialization, sdata.time_follower/g, sdata.time_leader/g, g, stop_eps);
#endif
